...
| AC-33 | **FR-T14 free-Cypher → FR-G03 chain (pass-1 B-03)** — mock LLM (per FR-T16) emits a `cypher` tool call with `statement: "CREATE (n:Activity {id:'x'}) RETURN n"` inside the `graph_analyst` role. End-to-end assertion: (1) the tool dispatch invokes `runPassthrough` from `api/src/neo4j/read-only-session.ts`; (2) `runPassthrough` rejects (driver `AccessMode.READ` is the structural gate) with thrown `ValidationError("write_statement_rejected")`; (3) the dispatch layer converts the throw to `{ ok: false, error: { code: 'write_statement_rejected', … } }`; (4) the orchestrator emits the FR-G03 fixed string verbatim — character-exact `"This question is not answerable read-only — please use the explorer to make changes."`; (5) `tool_calls[]` audit shows one entry with `tool_name: 'cypher'` and `error_code: 'write_statement_rejected'`. Run for `CREATE`, `MERGE`, `SET`, `DELETE` statements (4 sub-cases). | n/a (server) | `api/__tests__/chat/cypher-write-rejection.integration.test.ts` — uses the real Neo4j integration test fixture (not a mock) for the driver-side gate; runs in the `bun test:integration` job |
