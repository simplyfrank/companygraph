...
> **Revision 2 (2026-07-05)** — addresses `review-requirements.md` (pass 1,
> verdict: revise). Every finding is resolved in place and tagged at the
> resolving requirement: **B-01** → FR-06/FR-07/FR-08/FR-10 + AC-08/AC-12
> (unknown-`:modelId` gate, `404 model_not_found`); **B-02** → NFR-01 reworded
> (scoping bounds the Activity set + intra-scope `PRECEDES` only — shared
> `Role`/`System` reads are unconditional); **C-01** → FR-12 (`DataTable`
> additive extension named as a decision); **C-02** → analytics path corrected
> to `api/src/analytics/graph.ts` (Motivation §1, FR-02, Dependencies);
> **C-03** → FR-04 + AC-04 (handoffs require both sides non-empty **and**
> disjoint); **C-04** → FR-03 + AC-03 (chain length in **nodes**; zero-edge
> model → all `criticalPath` 0); **C-05** → NFR-03 restated + FR-09 atomicity;
> **C-06** → FR-10 reachability anchored in this spec's own OpenAPI test;
> **C-07** → FR-09 export/import claim qualified + risk row 7. Nits N-01..N-05
> folded in (casing note, landed-dependency note, frontmatter status,
> NFR-05 timing note, exported-`api.*` citation). Setting frontmatter
> `status: "revised"` resolves **N-03**. No stable ID renumbered.
>
> **Finding-ID disambiguation:** the IDs above are `review-requirements.md`
> **pass-1** IDs. The pre-existing "C-01" tags retained inside FR-09, AC-06 and
> risk row 4 refer to the **earlier drafting-pass finding** (the `patchNode`
> attributes-clobber) and are kept un-renumbered per the stable-ID rule; the
> pass-1 review's C-01 (the `DataTable` catalog gap) is resolved in FR-12.
...
1. The blueprint pipeline is **author → graph → optimize → measure →
   ...
   (`api/src/analytics/graph.ts` — betweenness/degree/pagerank via
   `graphology`; path corrected, resolves C-02) but nothing **model-scoped**,
   nothing that ranks **activities
...
| FR-02 | **Centrality score** per activity, computed over the model-scoped `PRECEDES` subgraph (directed, Activity→Activity). The score is **betweenness centrality** (via the existing `graphology-metrics/centrality/betweenness` already vendored for `api/src/analytics/graph.ts` — path corrected, resolves C-02) normalised to `[0,1]` within the model (divide by the model's max betweenness; all-zero → all `0`). Rationale + component values are reported alongside (raw betweenness, in-degree, out-degree) so the score is explainable (XD-11). A model with 0 or 1 scoped activities yields centrality `0` for each (no crash). | must | XD-11, cto-analytics AN-1 pattern |
| FR-03 | **Critical-path position score** per activity: `1` if the activity lies on the model's **longest acyclic `PRECEDES` chain** (the critical path), else a graded `[0,1)` value = `(depth of the longest acyclic chain the activity participates in) ÷ (length of the model's critical path)`. **Chain-length unit + zero-edge case (resolves C-04):** length is counted in **nodes** — a chain of `k` activities has length `k`; a chain requires **≥ 2 nodes**, so an isolated activity forms no chain. (C-04 required the unit be pinned and recommended edges; **node count** is chosen instead so the unit matches the `criticalPathLength` evidence field's natural reading — both are valid `[0,1]` gradings, the choice only shifts the graded fraction by the ±1 offset, and the boundary behavior below is identical.) When the model has activities but **zero intra-scope `PRECEDES` edges**, the critical-path length is `0` and **every `criticalPath` sub-score is `0`** (mirrors the FR-02/FR-04 all-zero convention; no division by zero — the graded formula is guarded by `criticalPathLength === 0 → 0`). Per-activity component evidence carries `{onCriticalPath, longestChainDepth, criticalPathLength}` so the graded value is explainable. The **longest-acyclic-chain computation is depth-bounded DFS with budgeted truncation** reusing the cto-analytics algorithm contract (resolves OQ-1 default): **depth cap = 20, path-count budget = 1000 candidate paths, wall-clock budget = 4 s**. A **cyclic** `PRECEDES` subgraph is flagged (`hasCycle: true`) but not crashed — the longest acyclic sub-chain is still reported; on budget exhaustion the response carries `{truncated: true, truncationReason: "depth_cap"\|"path_budget"\|"wall_clock"}` and scores are computed against the longest partial found. **Casing note (resolves N-01):** this surface deliberately uses en-US camelCase (`hasCycle`, `truncated`, `truncationReason`) — it reuses the cto-analytics FR-06 algorithm **contract** (caps + truncation surface), not its byte-level field names (`has_cycle`/`truncation_reason`/`longest_partial`); the partial chain itself is **not** exposed in `meta` — its per-activity projection (`longestChainDepth`, `criticalPathLength`) lives in the FR-03 evidence block instead. | must | XD-11, cto-analytics FR-06 pattern |
| FR-04 | **Handoff-density score** per activity = a normalised measure of how many **role/system boundaries cross** at that activity in the process flow. Concretely: `handoffCount(a) = ` the number of `(predecessor p —PRECEDES→ a)` transitions where `p` and `a` have **non-empty and disjoint executing-role sets** (a role handoff) **plus** the number where they have **non-empty and disjoint used-system sets** (a system handoff), summed over all model-scoped predecessors, **plus** the same over successors. **Empty-set rule (resolves C-03):** a role (resp. system) handoff is counted **only when both sides' sets are non-empty and disjoint** — an activity with zero executing roles (or zero used systems) contributes and receives **no** handoffs on that dimension. Empty sets are vacuously disjoint from everything; counting them would rank an **under-modeled** activity spuriously high, the opposite of trustworthy evidence (XD-11). Reported normalised to `[0,1]` within the model (divide by the model max; all-zero → all `0`) with the raw `handoffCount` and its role/system breakdown as component evidence (XD-11 explainability). An activity with no `PRECEDES` neighbours scores `0`. | must | XD-11 |
...
| FR-06 | **Scores endpoint** `GET /api/v1/models/:modelId/key-activities` (model-scoped path, following the `model-workspace-core`/`story-spec-core` route convention) runs FR-01–FR-05 **live** (no cache subsystem — this spec computes on request at `retail-mini`/single-model scale, matching cto-analytics DD-03 "serve live") and returns the ranked list plus a `meta` block `{ activityCount, hasCycle, truncated?, truncationReason?, weights }`. **Model-existence gate (resolves B-01):** every route in FR-06/FR-07/FR-08 gates on model existence **first** — an unknown `:modelId` returns `404 model_not_found` (**reusing** the existing `model-workspace-core` FR-13 code already in `ERROR_CODES`; no new code needed) **before** `scopedNodeIds` is consulted, whose empty set for an unknown model would otherwise be indistinguishable from a genuinely empty model. An **existing** model with zero scoped activities returns `200` with an empty ranked list and `meta.activityCount: 0` (the AC-12 empty state renders from this response, never from an unknown model). zod-validated at the boundary; `{error:{code,message,details?}}` envelope; appears in `GET /api/v1/openapi.json`. | must | XD-11, NFR-11 |

### Manual key-activity marking (XD-03)

| ID | Requirement | Priority | Source |
|----|-------------|----------|--------|
| FR-07 | **Mark as key** `POST /api/v1/models/:modelId/key-activities/:activityId/mark` sets `keyActivity` **score evidence** on the target `Activity` (XD-03: an attribute, not a new label). The stored evidence is a JSON object `{ marked: true, markedAt: <ISO>, scoreSnapshot: { centrality, criticalPath, handoff, composite }, rank: <int at mark time> }` written under the activity's open `attributes` map at key **`keyActivity`** (so it round-trips through the graph-core export/import — under the current permissive Activity attribute schema, see FR-09's C-07 qualification — and never touches the compile-time schema). Gate sequencing (B-01): an unknown `:modelId` returns `404 model_not_found` **first** (the FR-06 model-existence gate); only then is `:activityId` checked — it **must** be a model-scoped activity of `:modelId` (else `404 activity_not_found`; a cross-model activity is not found under this model path). Returns `200` + the updated activity's rank row. The write is **partial** — it must **not** clobber the activity's other `attributes` (FR-09 mechanism). | must | XD-03 |
| FR-08 | **Unmark (reversible)** `DELETE /api/v1/models/:modelId/key-activities/:activityId/mark` **removes** the `keyActivity` key from the activity's `attributes` map (restoring the exact prior attribute state — marking is fully reversible per XD-03), returns `204`. Idempotent: unmarking an unmarked activity returns `204` (no-op, not `404`). Re-marking after unmark writes a fresh snapshot at the then-current scores (marking is recomputable, XD-03). Gate sequencing as FR-07 (B-01): unknown `:modelId` → `404 model_not_found` first, then `404 activity_not_found` for a non-scoped activity. | must | XD-03 |
| FR-09 | **Attribute-preserving write mechanism (resolves C-01).** The core `patchNode` primitive replaces the **whole** `attributes` map (verified: `api/src/storage/nodes.ts` `SET n.attributes_json = $attrsJson`) and runs `assertAttributesMatchSchema` — so a naive `PATCH /nodes/Activity/:id {attributes:{keyActivity}}` would **clobber** the activity's other attributes and could trip attribute-schema enforcement. The mark/unmark writes therefore go through a **dedicated model-scoped storage function** (`api/src/storage/key-activities.ts`) that reads the current `attributes_json`, sets/deletes only the `keyActivity` key in-process, and writes the merged map back in a single parameterized Cypher `SET` — preserving every other attribute. **Atomicity (resolves C-05):** the read → merge → write sequence is **atomic per activity** (lock-first or a single transaction — the exact mechanism is a design decision), so a concurrent attribute write cannot be lost to the read-merge-write window. The generic `createNode`/`patchNode` primitives stay **byte-for-byte unchanged**. `keyActivity` is stored inside the **open `attributes` map** (not a top-level Neo4j property, not a registry-declared required attribute), so it is exempt from attribute-schema `required` enforcement. **Export/import qualification (resolves C-07):** the "survives export/import" claim holds **under the current permissive Activity attribute schema** — the graph-core import path (`upsertNode`) runs `assertAttributesMatchSchema`, which enforces declared `required` keys but admits undeclared ones; if a future **strict** Activity attribute schema (rejecting undeclared keys) ever lands, `keyActivity` must be declared in it or the import round-trip breaks (risk row 7). | must | XD-03, graph-core storage contract |

### API contract

| ID | Requirement | Priority | Source |
|----|-------------|----------|--------|
| FR-10 | Every route in FR-06, FR-07, FR-08 is mounted under `/api/v1/`, zod-validated at the boundary, and appears in `GET /api/v1/openapi.json` (generated from the same zod definitions — no hand-maintained copy). New error codes are added to the closed `ERROR_CODES` enum (`api/src/errors.ts`) as **additive** (non-breaking) changes: at minimum `activity_not_found` (a `:activityId` that is not a model-scoped `Activity` of `:modelId`). The unknown-model case **reuses** the existing `model_not_found` code (`model-workspace-core` FR-13) — it is consumed, **not** re-added (B-01). Every added code is reachable from ≥1 route; **this discipline is anchored in this spec's own test (resolves C-06 — there is no repo-wide `envelope.test.ts` reachability assertion):** AC-08's `key-activity-openapi.integration.test.ts` asserts the new code(s) appear in the OpenAPI error-envelope enum **and** are returned by a real request. No unreachable "reserved" code is added. | must | NFR-11, house rule |
...
|----|-------------|--------|
| NFR-01 | **Model isolation (reworded — resolves B-02).** All scoring reads (FR-01–FR-06) and the mark/unmark scope check (FR-07, FR-08) are scoped by `model-workspace-core`'s `scopedNodeIds(driver, :modelId)` helper (consumed, never re-implemented). Model-scoping bounds the **`Activity` set** (only activities whose ids are in `scopedNodeIds` are scored) and the **intra-scope `PRECEDES` edges** (a `PRECEDES` edge counts only when **both** endpoints are scoped activities; an edge to an out-of-model activity is excluded from centrality/critical-path/handoff). A scoped activity's `EXECUTES` (Role→Activity) and `USES_SYSTEM` (Activity→System) edges to **shared `Role`/`System` reference nodes are read unconditionally** — `Role`/`System` are cross-model reference nodes that are *never* in the scoped set (`model-workspace-core` DEC-01), so filtering those edges by `scopedNodeIds` would zero out every FR-04 handoff. Cross-model isolation is guaranteed because the **activity endpoints** are scoped, not because `Role`/`System` are. A request for model A never returns or marks a model-B-only activity. | XD-06, `model-workspace-core` FR-18 |
...
| NFR-03 | **Reversibility invariant (restated — resolves C-05)** (XD-03): unmarking (FR-08) removes **only** the `keyActivity` key; **all sibling attributes as of unmark time are preserved byte-for-byte** (no residue key, no altered or reverted siblings). A sibling attribute legitimately edited *between* mark and unmark keeps its edited value — unmark never restores pre-mark sibling state (the earlier "byte-equal to before the mark" wording was over-strong and would have required reverting interim edits). Verified by a round-trip test (AC-07, whose fixture does not edit siblings in between and therefore also observes byte-equality with the pre-mark map). | XD-03 |
...
| AC-08 | **Model isolation + authz + error contract**: seed two models each with their own activities; `GET /api/v1/models/:A/key-activities` scores only model-A activities and excludes model-B-only ones; marking a model-B activity under model A's path → `404 activity_not_found`; a request to **any of the three routes with an unknown `:modelId`** → `404 model_not_found` (B-01), while an existing model with zero scoped activities → `200` with empty `rows` + `meta.activityCount: 0`; a session without `key_activity:write` gets `403` on the mark `POST`/unmark `DELETE`, `key_activity:read` gets `200` on the GET; the `business_architect` role resolves both permissions; no new route is `public`; new routes + error codes appear in `GET /api/v1/openapi.json` **and** the OpenAPI test asserts each newly added code (`activity_not_found`) is both in the error-envelope enum and returned by a real request (C-06) (FR-01, FR-06, FR-07, FR-10, FR-11, NFR-01) | server (bun test + Neo4j) | `api/__tests__/key-activity-scope-authz.integration.test.ts` + `api/__tests__/key-activity-openapi.integration.test.ts` |
...
| AC-12 | With no scoped activities in the active model — i.e. a `200` response with empty `rows` + `meta.activityCount: 0` (FR-06's B-01 contract; an unknown model is a `404` and renders the **error** state, never this one) — KeyActivityBoard shows the empty state (a "no activities to score" message pointing to authoring) and no ranking table (FR-12 empty) | macOS Chrome (mouse+kb) | `pwa/src/__tests__/key-activity-board-states.test.tsx` |
...
> **Build-order dependency — LANDED (note refreshed, resolves N-02).** The
> `model-workspace-core` surfaces below are all **on disk and landed** as of this
> revision (verified 2026-07-05: `api/src/storage/model-scope.ts`,
> `pwa/src/context/ActiveModelContext.tsx`, `pwa/src/views/model/`,
> `api/src/scripts/seed-rbac-roles.ts` all exist — `model-workspace-core`
...
> have shipped). Design must treat these as **landed interfaces**, not pending
> ones; no external build-order precondition remains.
...
- **Existing graph analytics vendored libs** (`graphology` + `graphology-metrics/centrality/betweenness`, already imported by `api/src/analytics/graph.ts` — path corrected, resolves C-02): reused for the model-scoped centrality (FR-02). This spec builds its **own** model-scoped subgraph (not the whole-graph `fetchGraphFromNeo4j`); the `buildGraphologyGraph(nodes, edges)` helper (exported at `api/src/analytics/graph.ts:42`) **does** accept an arbitrary node/edge list (verified — the reuse question in risk row 6 is answered "yes").
...
- **PWA shell + catalog** (`pwa/src/views/index.tsx` `model`-surface dispatch, `pwa/src/components/{DataTable,Card,Button,Modal,SidePanel}.tsx`, `pwa/src/views/_shared.tsx` `Loading`/`ErrorState`, `pwa/src/styles/companygraph/tokens.css`, `scripts/design-conformance.ts`, and `pwa/src/api.ts` — via its **exported `api.*` methods**; the internal `json<T>()` helper is module-private (line 49, not exported) and is **not** a consumable interface (N-05). KeyActivityBoard's fetches ship as new exported `api.*` methods alongside the existing ones).
...
| 6 | **Design item — scoring-module home + graphology reuse (reuse question now ANSWERED, C-02).** FR-01/FR-02 place the model-scoped read + scoring at `api/src/storage/key-activities.ts` (+ a pure scoring helper). `buildGraphologyGraph(nodes, edges)` at `api/src/analytics/graph.ts:42` **does** accept an arbitrary node/edge list (verified during the C-02 path correction) — reusable as-is. | Cosmetic placement only. | Design confirms placement (recommend a pure `api/src/derive/key-activity-score.ts` for the Neo4j-free math so it is unit-testable against a fixture, mirroring story-spec's DD-01). Not a requirements blocker. |
| 7 | **C-07 — export/import round-trip of `keyActivity` depends on the Activity attribute schema staying permissive.** The graph-core import path (`upsertNode`) runs `assertAttributesMatchSchema`; today that validator admits undeclared keys, so `keyActivity` round-trips. A future strict (`additionalProperties: false`-style) Activity attribute schema would reject `keyActivity` on import while the FR-09 write path (which bypasses the validator) kept working — a silent asymmetry. | An ontology change outside this spec could break export→import of marked activities. | FR-09 qualifies the claim explicitly ("under the current permissive Activity attribute schema"). Whichever spec ever introduces a strict Activity attribute schema must declare `keyActivity` (or migrate it) — recorded here so that spec's review can catch it. Optionally, design may add an export→import round-trip case to the AC-06/AC-07 test to pin the assumption. |
</content>
