...
revision: 3
revision_note: "revision 3 (2026-07-04): all 7 design open questions resolved by owner decision (RD-1..RD-7); FR-08/FR-10/FR-11/FR-11a deferred to cto-analytics-reporting; see §Resolved Decisions."
size: "medium"
...
The seven design-phase open questions were resolved by owner decision on
2026-07-04 (delegated, autonomous completion). The authoritative ledger is
**design.md §10 (RD-1..RD-7)**; this section records the propagation into
requirements. RD-N ⇔ former OQ-N.

| RD | Decision | Requirements sites |
|----|----------|--------------------|
| RD-1 | Abstract Neo4j behind a shared read-only module (`api/src/neo4j/read-only-*.ts`); no direct driver in `api/src/analytics/`. | NFR-02, AC-11, FR-09 |
| RD-2 | Build FR-04's weighted `depth × systems × roles`; proxy is interim. | FR-04 |
| RD-3 | Keep as-built short route names (`matrix`, `ai`); new tabs use FR names. | FR-02, FR-07, AC-02, Scope |
| RD-4 | Rule-based AI candidates; adopt chat's `leverage_score` ranking. | FR-07 |
| RD-4a | Ratify as-built vocabulary (`repetition`/`data_richness` enum `"med"`, `leverage_score` ≥ 0.5); fix `"medium"`→`"med"`. | FR-07, AC-07, AC-15, Dependencies |
| RD-5 | Detokenize `Complexity.tsx`/`Ai.tsx` chart hex → tokens, in the pending tasks. | (design DD-08; tasks T-10/T-13/T-21) |
| RD-6 | Defer FR-08/FR-10/FR-11/FR-11a (PDF, scheduler, settings/audit, snapshot) to follow-up spec `cto-analytics-reporting`; nothing deleted. | FR-08, FR-10, FR-11, FR-11a, FR-04, FR-09, AC-10, Scope, Dependencies |
...
| ID | Requirement | Priority | Story |
|----|-------------|----------|-------|
| FR-04 | **Complexity score per journey** — score = `depth(PRECEDES chain) × distinct(USES_SYSTEM) × distinct(EXECUTES_role)`. **RESOLVED 2026-07-04 (RD-2):** this weighted formula is **canonical** (T-10 builds it); the as-built `activities + fanOut + fanIn` proxy stays only as an interim visible label until T-10 lands. Rendered in a sortable table at `#/analytics/complexity`. Hover (long-press on touch) on a score reveals the formula + the three component values. Score weights default `1.0` each. **RESOLVED 2026-07-04 (RD-6):** the *tunable* settings pane (`#/analytics/settings`) + `analytics_settings` table are **deferred with FR-11** to `cto-analytics-reporting`; until then the weights are **code-default constants** (all `1.0`) surfaced read-only by the `Settings.tsx` pane T-10 builds. (Formerly "computed nightly (FR-08 scheduler)" — the scheduler is deferred with FR-10; BUILD-set reports serve live per DD-03.) | must | AN-2.1 |
...
| ID | Requirement | Priority | Story |
|----|-------------|----------|-------|
| FR-07 | **AI-candidate filter — configurable attribute coupling** (resolves pass-1 C-04). `#/analytics/ai` (**RESOLVED 2026-07-04, RD-3:** short as-built route name kept; was `#/analytics/ai-candidates`) reads the filter attribute keys + match values + a `leverage_score` threshold from an `analytics_ai_candidate_definition` row (**RESOLVED 2026-07-04, RD-4 + RD-4a:** rule-based, adopting chat's `leverage_score` ranking so analytics ≡ chat; the definition ratifies the **as-built vocabulary** — `repetition`/`data_richness` enum strings + `leverage_score` per `shared/seed/retail-mini-enriched.json` and `api/src/chat/tools/ai-candidates.ts`). **Defaults** (RD-4a): `{repetition_key: "repetition", repetition_match: "high", richness_key: "data_richness", richness_match: "high", leverage_score_key: "leverage_score", leverage_min: 0.5}`. Filter applies `attributes[repetition_key] == repetition_match AND attributes[richness_key] == richness_match AND attributes[leverage_score_key] >= leverage_min`, results sorted by `leverage_score` DESC. (The former literal defaults `{repetitive_key:"repetitive", repetitive_match:true, richness_match:"high"}` matched **zero** rows on the enriched seed — the RD-4a fix.) Results render in a table with the activity + parent journey + bound systems + bound roles + `leverage_score`. **CSV export**: button produces a UTF-8 file with byte-order-mark (`\xEF\xBB\xBF`), CRLF line endings, RFC 4180 quoting (resolves pass-1 C-02). Empty-result state shows explicit copy: `"no activities tagged yet — see ontology-manager to register repetition + data_richness + leverage_score (or your configured) attributes on Activity"` (NOT a blank screen; RD-4a names the real as-built attributes). | must | AN-3.1 |
| FR-08 | **priority: deferred** (2026-07-04 owner decision, RD-6): PDF export + graph-state hash add a runtime PDF dependency (library choice `@react-pdf/renderer`/`pdfkit`/`puppeteer`) + the NFR-05 hash-protocol module, warranting explicit design sign-off; **deferred to follow-up spec `cto-analytics-reporting`**; nothing deleted — the full requirement text below is carried forward verbatim as that spec's input. **Executive summary PDF export — cache-snapshot-grounded hash** (resolves pass-1 B-02 + B-04). `#/analytics/exec-summary/export` is a PWA-side **launcher route** that hits the API endpoint `GET /api/v1/analytics/exec-summary.pdf`; **no client-side PDF rendering**. The API endpoint generates the PDF server-side and returns `application/pdf`. PDF covers: top 5 journeys by complexity (with sub-scores), top 3 consolidation candidates (activities with most `USES_SYSTEM` edges), top 3 AI candidates. **Hash basis** (precise field list, derived from `graph-core`'s actual export shape — edge `updatedAt` does NOT exist; edges are immutable post-creation in graph-core): hash input is `{snapshot_id, nodes, edges, weights}` where: (a) `snapshot_id = analytics_journey_scores.last_run_at` (the cache run id — pinning the hash to the cache snapshot, NOT the live graph); (b) `nodes` is the cache-snapshot list of `{id, label, attributes_parsed, updatedAt}` rows; (c) `edges` is the cache-snapshot list of `{id, type, fromId, toId, attributes_parsed, createdAt}` rows (no `updatedAt`); (d) `weights` is the `analytics_settings` weight row at PDF-render time (depth_weight, system_weight, role_weight). PDF body includes `last_run_at` (the cache snapshot timestamp) so a verifier can distinguish "same graph, different cache" from "different graph". **Hash placement**: visible page-1 footer in monospace (e.g. `"graph-state hash: 4f3a…b9c1 · cache snapshot: 2026-05-22T18:30:00Z"`) AND PDF metadata `/Subject` field (the canonical-data location). **Verifiability**: a reader can re-derive the hash by calling the new `GET /api/v1/analytics/snapshot/:last_run_at` endpoint (FR-11a) which returns the exact cache contents. | must | AN-3.2 |

### Backend

| ID | Requirement | Priority | Story |
|----|-------------|----------|-------|
| FR-09 | **Analytics REST endpoints** — under `/api/v1/analytics/*`: `GET /systems`, `GET /matrix` (RD-3 short name), `GET /consolidation`, `GET /complexity`, `GET /single-system-journeys`, `GET /critical-paths`, `GET /ai-candidates` (the 7 BUILD-set report GETs), plus `GET /exec-summary.pdf` (**deferred with FR-08, RD-6** — registered in `cto-analytics-reporting`). **RESOLVED 2026-07-04 (RD-1):** all read-only; all graph reads go through the shared read-only Neo4j module (`api/src/neo4j/read-only-*.ts`), **not** a self-HTTP `POST /api/v1/query/cypher` hop (reworded per NFR-02/RD-1 — analytics needs the uncapped full graph). | must | AN-1..AN-3 |
| FR-10 | **priority: deferred** (2026-07-04 owner decision, RD-6): the nightly scheduler + `analytics_*` cache tables + staleness/`degraded` envelope + `?refresh=true` lock protocol are a scheduler/cache subsystem warranting explicit design sign-off; **deferred to follow-up spec `cto-analytics-reporting`**; nothing deleted. The BUILD-set reports serve **live** (DD-03) at `retail-mini` scale until the follow-up lands the cache. Full text carried forward verbatim: **Nightly precompute scheduler + on-demand refresh** — a job runs nightly at the operator-configured cron expression (default `0 2 * * *` in the operator's `TZ` env var) that computes complexity scores for every journey, integration density for every system, and AI-candidate flags. Results land in `analytics_journey_scores` + `analytics_system_metrics` + `analytics_ai_candidates` (SQLite, isolated from `chat-interface`'s SQLite). Endpoints serve from these tables; if stale (`last_run_at > 25h ago`), endpoints return `{degraded: true, last_run_at}` in the response envelope but still serve the stale data. **On-demand refresh** (resolves pass-1 N-05): every endpoint accepts `?refresh=true` which triggers a lock-protected on-demand recompute; a second concurrent `?refresh=true` call returns the in-progress run's eventual result (no double-execution). **Schema-coupling validation** (resolves pass-1 C-04): before each precompute run, the scheduler validates that the `analytics_ai_candidate_definition` row's `repetitive_key` and `richness_key` exist in `ontology-manager`'s `Activity` attribute schema (`GET /api/v1/schema`). On mismatch, the precompute skips the AI-candidate pass and writes a structured banner row to `analytics_alerts` table (rendered in PWA Overview): `"AI-candidate definition references attribute '<key>' which is not registered on Activity — visit ontology-manager"`. | must | AN-2.1 (implied) |
| FR-11 | **priority: deferred** (2026-07-04 owner decision, RD-6): the settings subsystem + `GET`/`PATCH /settings` + `analytics_settings_audit` add a settings/audit subsystem warranting explicit design sign-off; **deferred to follow-up spec `cto-analytics-reporting`**; nothing deleted. Until then, complexity weights + the AI-candidate definition ship as **code-default constants** read by the BUILD-set reports (design §10.2), so no BUILD task depends on this deferred subsystem. Full text carried forward verbatim: **Settings table + audit** — `analytics_settings` stores complexity score weights (depth_weight, system_weight, role_weight; all defaults `1.0`) + scheduler cron expression + PDF brand options + AI-candidate definition. `GET /api/v1/analytics/settings` + `PATCH /api/v1/analytics/settings`. **Audit on every PATCH** (resolves pass-1 C-01): writes a row to `analytics_settings_audit (ts, before, after, actor)` so PDF readers comparing two hash headers can trace which weight change caused the drift. Same shape as `graph-core/FR-13` structured logging. | should | AN-2.1 |
| FR-11a | **priority: deferred** (2026-07-04 owner decision, RD-6): the cache-snapshot read endpoint exists only to re-derive FR-08's PDF hash and depends on FR-10's cache tables + FR-08's PDF — all deferred together; **deferred to follow-up spec `cto-analytics-reporting`**; nothing deleted. Full text carried forward verbatim: **Cache-snapshot read endpoint** — `GET /api/v1/analytics/snapshot/:last_run_at` returns the exact cache contents (`analytics_journey_scores`, `analytics_system_metrics`, `analytics_ai_candidates`, `analytics_settings` weights row, all keyed at `last_run_at`) used by FR-08's PDF generation. Used by external verifiers to re-derive the hash. | must | AN-3.2 |
...
| NFR-02 | **RESOLVED 2026-07-04 (RD-1):** All analytics graph reads route through the shared read-only Neo4j module (`api/src/neo4j/read-only-*.ts` — `read-only-session.ts` `runPassthrough` for capped reads, `read-only-graph.ts` `fetchGraph`/`runReadOnlyGraph` for the uncapped full-graph read analytics needs); **no direct `getDriver()`/`driver.session()` inside `api/src/analytics/`**. (Was: "all Cypher routes through `POST /api/v1/query/cypher`" — reworded because analytics needs the full graph, >1000 rows, which the passthrough endpoint's row cap would truncate, and the loopback self-HTTP hop buys nothing server-side.) Coverage test (AC-11) enforces. | architecture |
...
- **RESOLVED 2026-07-04 (RD-3 + RD-6):** the analytics REST endpoints split into a BUILD set and a deferred set. **BUILD set — 7 report GETs** (this spec): `systems`, `matrix` (RD-3 short name), `consolidation`, `complexity`, `single-system-journeys`, `critical-paths`, `ai-candidates`. **Deferred to `cto-analytics-reporting` (RD-6)**: `exec-summary.pdf` (FR-08), `GET/PATCH /settings` (FR-11), `GET /snapshot/:last_run_at` (FR-11a). (The original count was "Ten endpoints, 11 registrations" — the deferral moves 3 paths/4 registrations to the follow-up spec; resolves pass-1 N-02.)
...
| AC-02 | Domain↔system matrix at `#/analytics/matrix` (RD-3) renders rows × columns; cell links open the underlying activity list (FR-02) | all four | `pwa/src/__tests__/analytics-matrix.test.tsx` + integration: click any cell, expect `process-explorer-ui` filter URL with `?system_id=:sid&domain_id=:did` (binding param names per FR-02 / design §2 Open-3) |
...
| AC-07 | AI-candidate filter reads attribute keys + match values + `leverage_score` threshold from `analytics_ai_candidate_definition`; CSV export emits UTF-8 BOM + CRLF + RFC 4180 quoting; empty state shows the named copy (FR-07 — pass-1 C-02 + C-04; **RESOLVED 2026-07-04 RD-4a:** fixture + defaults use the real as-built vocabulary) | all four | `api/__tests__/analytics-ai-candidates.test.ts` — (a) default-definition test: seed activities from `retail-mini-enriched.json`, filter applies `repetition=="high" AND data_richness=="high" AND leverage_score>=0.5`, results sorted by `leverage_score` DESC (per RD-4a — the enriched seed populates the tab, not empty); (b) reconfigured-definition test: override the definition to `{repetition_key:"manual_repeat", richness_key:"info_density", richness_match:"rich", leverage_min:0.7}`, assert filter switches; (c) CSV byte test: first three bytes of CSV body are `EF BB BF`, line endings are `\r\n`, fields with commas are quoted per RFC 4180; (d) empty-state via `pwa/src/__tests__/analytics-ai-empty-state.test.tsx` |
...
| AC-10 | **RESOLVED 2026-07-04 (RD-6):** all **shipped** analytics endpoints (the 7 BUILD-set FR-09 report GETs) under `/api/v1/analytics/*` conform to the envelope (`graph-core/NFR-05`; FR-09, NFR-08). The deferred `/exec-summary.pdf`, `GET+PATCH /settings`, `GET /snapshot/:last_run_at` (FR-08/FR-11/FR-11a) roll into `cto-analytics-reporting`; AC-10 **closes at T-19** over the shipped set (was: closes at T-18 over all 10 endpoints). | n/a (server) | `api/__tests__/analytics-envelope.test.ts` — asserts every shipped `/api/v1/analytics/*` report GET returns the NFR-08 envelope |
| AC-11 | **RESOLVED 2026-07-04 (RD-1):** All analytics graph reads go through the shared read-only Neo4j module (`api/src/neo4j/read-only-*.ts`); no direct `getDriver()`/`driver.session()` inside `api/src/analytics/` source (NFR-02) | n/a (codebase) | `api/__tests__/analytics-no-direct-driver.test.ts` — grep over `api/src/analytics/` for `getDriver(` / `driver.session(` (direct-driver use) and for write-helper imports; assert zero hits (reads are expected to import from `api/src/neo4j/read-only-graph.ts`) |
...
| AC-15 | Empty-state copy for AI candidates names `ontology-manager` and the attribute schema registration step, naming the **real as-built attributes** `repetition + data_richness + leverage_score` (FR-07 — RD-4a) | all four | Part of `pwa/src/__tests__/analytics-ai-empty-state.test.tsx` — assert the literal string (revised wording per RD-4a) |
...
| `ontology-manager` | **Mandatory soft dependency** — the AI-candidate filter (FR-07) hinges on `Activity.repetition` (`"low" \| "med" \| "high"`), `Activity.data_richness` (`"low" \| "med" \| "high"`), and `Activity.leverage_score` (number) being registered as attributes via `ontology-manager` (**RESOLVED 2026-07-04 RD-4a:** the enum value is `"med"`, matching `shared/seed/retail-mini-enriched.json` + `api/src/chat/tools/ai-candidates.ts` — was the mismatched `"medium"`, and the key is `repetition` not the former boolean `repetitive`). If not registered, the empty-state copy in FR-07 surfaces. |
...
| PDF library — `pdfkit` (Node-native), `puppeteer` (heavy but flexible), or `@react-pdf/renderer` (React-native, fits the React stack) | **RESOLVED 2026-07-04 (RD-6):** the PDF-library choice is **deferred with FR-08** to the follow-up spec `cto-analytics-reporting` (it decides the library). Recommendation carried forward: `@react-pdf/renderer` to keep the codebase React-coherent. No PDF dependency is added by this spec. |
