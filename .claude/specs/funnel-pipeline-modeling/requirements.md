...
| NFR-03 | **Idempotent, additive registry writes.** Registering `Funnel`/`Stage`/`HAS_STAGE`/`CONVERTS_TO` is idempotent (re-run adds nothing, errors nothing) and additive — it never deprecates or mutates existing registry labels/edges (e.g. the core `PRECEDES`), and never touches retail Business Model #1's subgraph. | XD-01, house data-integrity |
...
|----|-----------|-----------|--------------|
| AC-01 | Registering the `Funnel` node label via `POST /api/v1/ontology/node-labels` succeeds (201), `GET /api/v1/ontology/node-labels/Funnel` returns it, and a second registration is a no-op (no duplicate, no error); `NODE_LABELS` in `shared/src/schema/nodes.ts` is unchanged (FR-01, NFR-01) | server (bun test + Neo4j) + CLI | `api/__tests__/funnel-registry.integration.test.ts`; manual: `git diff shared/src/schema/nodes.ts` — expect no additions |
