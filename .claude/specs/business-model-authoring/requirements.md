...
| FR-12 | **Model-scoped + reload survival**: the wizard/canvas only ever author into and render the **active** `BusinessModel`; switching the active model (via the shell context) resets/refetches for the new model; deep-linking `#/model/canvas` and reloading re-renders for the persisted active model (persistence is `model-workspace-core` FR-15; this view consumes it). No cross-model leakage of **model-scoped** structure: every read, the FR-07 authoring write, and the mwc domain-attach (FR-03) are scoped by `:modelId` = the active model; new `Domain`s are `IN_MODEL`-scoped to it via mwc's route (not by this spec). Shared/global reference nodes (`Role`/`System`/`Location`, DEC-01(a)) are intentionally cross-model and are not "leakage." In-progress, uncommitted wizard input is **not** persisted across reload (only committed graph state survives — an explicit non-goal, surfaced to the user as a "commit each step" model). | must | UX-06, `model-workspace-core` FR-15 |

### API contract

| ID | Requirement | Priority | Source |
|----|-------------|----------|--------|
| FR-13 | The one new route (FR-07 `POST /api/v1/models/:modelId/authoring/apply`) is mounted under `/api/v1/`, zod-validated at the boundary, and appears in `GET /api/v1/openapi.json` (generated from the same zod definitions — no hand-maintained copy). It reuses existing `ERROR_CODES` — `invalid_payload`, `attribute_violation`, `edge_endpoint_label_mismatch`, `model_not_found` (all four verified present in `api/src/errors.ts`); design decides whether any genuinely new failure mode warrants an additive (non-breaking, NFR-11) code (N-01). All template-clone and story/AC calls go to **existing** routes (`model-workspace-core`, `story-spec-core`) — this spec introduces exactly one new endpoint. | must | NFR-11, blueprint Scope |
| FR-14 | **Route-permission mapping**: the **one new** route this spec adds — `POST /api/v1/models/:modelId/authoring/apply` — is registered in `api/src/auth/rbac-permissions.ts` (`ROUTE_PERMISSIONS`) requiring **`model:write`** (structural authoring into a model is a model write, per `model-workspace-core` FR-11/FR-12's `model:*` vocabulary — no new permission is minted). **The template-clone step (FR-08) additionally exercises the *existing* `POST /api/v1/models/:modelId/module-instances` route, which `model-workspace-core` design §5 (design.md line 716) maps to `module:write`** — *not* `model:write`. So the feature's `must` write paths require **both** `model:write` (the authoring/apply route) and `module:write` (the clone route). Both permissions are already carried by the `business_architect` role (`model-workspace-core` FR-11 grants `model:read/write` + `module:read/write`), so the persona is fully covered; this spec adds **no** new RBAC role or permission and re-maps **no** existing route (the `module-instances` → `module:write` mapping is owned and asserted by `model-workspace-core`). Auth is enforced **only** by the central router gate (`api/src/router.ts`) + `api/src/auth/` — no per-route auth check. No new route is `public`. **Resolves: B-02.** | must | house rule, XD-08, `model-workspace-core` FR-11/FR-12, mwc design §5 |
...
- One new model-scoped authoring endpoint `POST /api/v1/models/:modelId/authoring/apply` that lands **journey/activity/role structure** via the existing `POST /api/v1/import` writer (domain-attach + `IN_MODEL` is delegated to mwc's `POST /models/:id/domains`, C-02).
...
- `model:write` route-permission mapping for the one new route.
...
| AC-08 | The FR-07 authoring endpoint lands structure via the existing `POST /api/v1/import` two-phase writer (reusing `realImport`'s core): a batch with one bad row returns HTTP 200 with the import result shape `{ imported:{nodes,edges}, errors:[{section,index,code}] }` (the failing row(s) under `errors[]`, valid rows persisted — collect-and-continue, C-03); each new node/edge carries a **server-generated UUIDv7** so re-submitting the same step upserts idempotently rather than colliding (C-04); the endpoint creates **no** new node label or edge type and writes **no** `IN_MODEL` (FR-07, NFR-01, NFR-02) — **Resolves: C-03, C-04** | server (bun test + Neo4j) | `api/__tests__/authoring-apply.integration.test.ts` |
...
| AC-10 | Router gate enforces the mappings for **both** `must` write paths: (a) a session without `model:write` gets 403 on `POST /api/v1/models/:modelId/authoring/apply` and with `model:write` it succeeds; (b) a session without `module:write` gets 403 on the clone route `POST /api/v1/models/:modelId/module-instances` (mwc-owned mapping, asserted still in force) and with `module:write` it succeeds; the `business_architect` role carries **both** permissions so a full run succeeds; neither route is `public`; the `authoring/apply` route + any error codes appear in `GET /api/v1/openapi.json` (FR-13, FR-14) — **Resolves: B-02** | server (bun test) | `api/__tests__/authoring-authz.test.ts` + `api/__tests__/authoring-openapi.integration.test.ts` |
| AC-11 | Wizard-complete ready state: `ModelCanvas` renders the active model's authored structure on the JourneyCanvas/react-flow canvas (journeys as lanes, activities ordered, roles/systems/locations as layers) read from active-model-scoped reads, with an "Edit in wizard" affordance that reopens the wizard at the relevant step (FR-09, FR-11 ready) | macOS Chrome (mouse+kb), macOS Safari (trackpad+kb) | `pwa/src/__tests__/model-canvas.test.tsx` |
...
- **Central router gate** (`api/src/router.ts`) + `ROUTE_PERMISSIONS` (`api/src/auth/rbac-permissions.ts`): the one new route dispatched + auth-gated here; no per-route auth.
