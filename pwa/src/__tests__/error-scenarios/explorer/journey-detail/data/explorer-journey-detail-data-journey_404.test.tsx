...
describe("Explorer · Journey-detail · Data · JOURNEY 404", () => {
  beforeEach(() => {
    cleanup();
    vi.restoreAllMocks();
  });

  test("(1) Invalid journey ID", async () => {
      ...
      return new Response(JSON.stringify({ rows: [] }), { status: 200 });
        ...
        const route = { surface: "explorer", tab: "journey-detail", params: { "journeyId": "invalid-journey-id" } };
    await act(async () => {
  ...
  test("(2) Non-existent journey ID", async () => {
        vi.spyOn(globalThis, "fetch").mockImplementation(async (url, init) => {
      const u = String(url);
      if (u.includes("/api/v1/query/cypher") && init?.method === "POST") {
        return new Response(JSON.stringify({ error: { code: "not_found", message: "Resource not found" } }), { status: 404 });
      ...
      return new Response(JSON.stringify({ rows: [] }), { status: 200 });
        ...
        const route = { surface: "explorer", tab: "journey-detail", params: { "journeyId": "0189abcd-1234-5678-90ab-cdef01234567" } };
    await act(async () => {
      render(<ExplorerJourney route={route} />);
    });
        // For detail mode with invalid entityId, expect NotFoundPanel or error state
