...
describe("Ontology · Erd · Network · GRAPH DATA FAILURE", () => {
  beforeEach(() => {
    cleanup();
    vi.restoreAllMocks();
  });

  test("(1) Network timeout during graph load", async () => {
        vi.spyOn(globalThis, "fetch").mockImplementation(async (url, init) => {
      const u = String(url);
      if (u.includes("/api/v1/query/cypher") && init?.method === "POST") {
        throw new Error("Error: Failed to load graph data");
      }      // Default fallback for unmatched URLs
      return new Response(JSON.stringify({ rows: [] }), { status: 200 });
    });
        const route = { surface: "ontology", tab: "erd", params: {} };
    await act(async () => {
    ...
    await waitFor(() => {
      ...
      if (errorState) {
        ...
        const mainHeader = headers.find(h => h.textContent?.toLowerCase().includes("entity relationship diagram"));
        if (mainHeader) {
