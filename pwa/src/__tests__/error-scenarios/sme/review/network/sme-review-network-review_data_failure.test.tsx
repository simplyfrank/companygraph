...
describe("Sme · Review · Network · REVIEW DATA FAILURE", () => {
  beforeEach(() => {
    cleanup();
    vi.restoreAllMocks();
  });

  test("(1) Network timeout during review data fetch", async () => {
        vi.spyOn(globalThis, "fetch").mockImplementation(async (url, init) => {
      const u = String(url);
      if (u.includes("/api/v1/query/cypher") && init?.method === "POST") {
        throw new Error("Error: Failed to fetch review data");
      }      // Default fallback for unmatched URLs
      return new Response(JSON.stringify({ rows: [] }), { status: 200 });
    });
        const route = { surface: "sme", tab: "review", params: { "journeyId": "journey-1" } };
    await act(async () => {
    ...
    await waitFor(() => {
      ...
      if (errorState) {
        ...
        const mainHeader = headers.find(h => h.textContent?.toLowerCase().includes("sme review"));
        if (mainHeader) {
